...
@Service
public class ProjectSquadMemberAddFundraiserService extends AbstractService<ProjectSquad, Member> {
	...
	@Override
	public void unbind() {
		Collection<UserAccount> fundraisersNotInProject;
		SelectChoices accounts;
		Tuple tuple;
		int projectId = super.getRequest().getData("projectId", int.class);

		fundraisersNotInProject = this.repository.findAccountOfFundraisersNotInProject(projectId);

		accounts = SelectChoices.from(fundraisersNotInProject, "username", null);
		tuple = super.unbindObject(this.member);
		tuple.put("accounts", accounts);
		tuple.put("account", null);
		super.unbindGlobal("projectId", this.project.getId());
		super.unbindGlobal("isFundraiser", true);
